...
gamma <- 2
phi <- gamma^2
phi_exp <- 1 # This is the phi parameter that is fed into the exponential conditional likelihood function and is what 
# the params is equal to

## Method 1: Conditional Likelihood
sims1 <- matrix(NA, nrow=50, ncol=1) # Restricting the number of sims, M=50, since my PC cannot handle large sims

## Obtain the estimates and populate sims1
for(i in c(1:50)){ 
  set.seed(i)
  
  Sigma_grid <- Matern(D, nu = nu, phi = phi_exp) # phi=1 (based on the "group2_conditional_likelihood.Rmd" sample 
  # simulations)
